...
if ( // 만약 speechSynthesis가 정의되어 있고, onvoiceschanged가 정의되어 있다면
    typeof speechSynthesis !== 'undefined' && speechSynthesis.onvoiceschanged !== undefined) {
    ...
    if (text.includes(coffee)) {
        ...
        * */
        ...
        if (textSplit.length > 1) {
            ...
            data.content = data.content + '<div class="orderList">' + '<div class="number">' + data.coffeeKind + '</div>' + '<div class="menu">' + coffee + '</div>' + '<div class="menu">' + coffeeCount + '</div>' + '<div class="menu">' + (coffeeCount * coffeePrice).toLocaleString('ko-KR') + ' <span>원</span></div>' + '</div>'
        }
    }
    return data;
}
        ...
        ajax(aoo, function (resp) {
            ...
            if (text.includes("주문하신") || text.includes("수정")) {
                ...
                var data = { //declartion of data object
                    content: '', coffeeKind: 0, coffeeCount: 0, price: 0
                };
                var menu = ["에스프레소", "아메리카노", "카푸치노", "카페라떼", "바닐라라떼", "카라멜마끼야또", "카페모카", "아포카토", "토마토주스", "키위주스", "망고스무디", "딸기스무디", "쿠키앤크림", "레몬에이드", "복숭아아이스티", "허브티", "말차라떼", "초콜릿라떼"];
